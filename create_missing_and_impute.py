...
from create_missing.draw import show_specify_line, show_change
...
def use_missing_creator(df_last, missing_rate, missing_columns, if_figure, if_write):
    # 排除时间戳
    ...
    # 是否展示图片
    if if_figure:
        show_specify_line(df_missing, missing_columns, 'red')

    return df_missing


def use_imputer(df_missing, missing_rate, missing_column, imputer_name, if_figure, if_write):
    if if_write:
    ...
    if if_figure:
        # show_change(df_origin, df_imputed, ["0", "1", "2", "3", "4", "5", "6"])
        show_change(df_missing, df_imputed, ['OT'])
...
if __name__ == '__main__':
    ...
    # show_specify_line(df_origin, ['OT'], 'green')
    df_last = df_origin
    ...
    for  missing_rate in missing_rate_list:
        ...
        df_missing = use_missing_creator(df_last, missing_rate, ["OT"], True, False)
        # df_missing = use_missing_creator(df_last, missing_rate, ["0", "1", "2", "3", "4", "5", "6"], False, False)
        # df_missing = use_missing_creator(df_last, missing_rate, ["0", "1", "2", "3", "4", "5", "6", "OT"], False, False)
        df_last = df_missing

        # use imputer
        methods = ["mean", "front", "knn", "xgboost", "miss_forest"]
        for method in methods:
            use_imputer(df_missing, missing_rate, "OT", method, True, False)
            # use_imputer(df_missing, missing_rate, "covariate", method, False, False)
            # use_imputer(df_missing, missing_rate, "all", method, False, False)