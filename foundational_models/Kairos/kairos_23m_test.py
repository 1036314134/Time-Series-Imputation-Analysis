...
def _load_kairos_forecastor():
    module_path = Path(__file__).resolve().parent / "kairos_23m_forecastor.py"
    module_name = "kairos_23m_forecastor_module"
    ...
    if not hasattr(module, "kairos_23m_forecastor"):
        raise AttributeError("kairos_23m_forecastor.py does not define kairos_23m_forecastor.")

    return module.kairos_23m_forecastor
...
def test_kairos_23m_forecastor(
    csv_relative_path,
...
if __name__ == "__main__":
    result_df, window_df = test_kairos_23m_forecastor(csv_relative_path="dataset/exchange_rate/exchange_rate.csv")
    print(f"Generated {len(result_df)} predictions across {len(window_df)} windows.")
    save_dataframe_to_csv(result_df, "results/exchange_rate_predictions_kairos_23m.csv")
    save_dataframe_to_csv(window_df, "results/exchange_rate_windows_kairos_23m.csv")
