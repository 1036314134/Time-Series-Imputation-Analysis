...
try:
    ...
    raise ImportError(
        "Kairos requires tsfm package. Please install tsfm to use Kairos_50m."
    ) from exc
...
def _resolve_local_model_dir(model_dir: Path) -> Path:
    ...
    raise FileNotFoundError(
        ...
        "Run kairos_50m_downloader.py first."
    )
...
def kairos_50m_forecastor(dataframe, forecast_length, num_samples=100, freq=None, device=None):
