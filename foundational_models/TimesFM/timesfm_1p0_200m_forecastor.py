...
LOCAL_MODEL_DIR = Path(__file__).resolve().parent / "timesfm_1p0_200m-pytorch"
_LEGACY_MODEL_CACHE = {}
...
def _ensure_local_model_dir(model_dir: Path):
    if not model_dir.exists() or not any(model_dir.iterdir()):
        raise FileNotFoundError(
            ...
            "Run timesfm_1p0_200m_downloader.py first."
        )
...
def _supports_legacy_timesfm():
    return _timesfm_pkg is not None and hasattr(_timesfm_pkg, "TimesFm")


def _build_legacy_checkpoint(model_dir: Path):
    if not hasattr(_timesfm_pkg, "TimesFmCheckpoint"):
        return None
...
def _load_legacy_model_via_hparams(backend: str, forecast_length: int):
    ...
    return _timesfm_pkg.TimesFm(hparams=hparams, checkpoint=checkpoint)


def _load_legacy_model_via_direct_init(backend: str, forecast_length: int):
    signature = inspect.signature(_timesfm_pkg.TimesFm)
    desired = {
        "context_len": 512,
        "horizon_len": int(forecast_length),
        "input_patch_len": 32,
    ...
    filtered = {k: v for k, v in desired.items() if k in signature.parameters}
    model = _timesfm_pkg.TimesFm(**filtered)

    if not hasattr(model, "load_from_checkpoint"):
        raise AttributeError("Legacy TimesFm model has no load_from_checkpoint method.")

    checkpoint_file = _resolve_checkpoint_file(LOCAL_MODEL_DIR)
    load_signature = inspect.signature(model.load_from_checkpoint)
    load_kwargs = {}
    if "checkpoint_path" in load_signature.parameters:
        load_kwargs["checkpoint_path"] = str(checkpoint_file)
    if "path" in load_signature.parameters:
        load_kwargs["path"] = str(checkpoint_file)
    if "checkpoint_dir" in load_signature.parameters:
        load_kwargs["checkpoint_dir"] = str(LOCAL_MODEL_DIR)
    if "local_dir" in load_signature.parameters:
        load_kwargs["local_dir"] = str(LOCAL_MODEL_DIR)
    if "repo_id" in load_signature.parameters:
        load_kwargs["repo_id"] = None

    model.load_from_checkpoint(**load_kwargs)
    return model
...
def _get_legacy_model(backend: str, forecast_length: int):
    ...
    if hasattr(_timesfm_pkg, "TimesFmHparams") and hasattr(_timesfm_pkg, "TimesFmCheckpoint"):
        model = _load_legacy_model_via_hparams(backend=backend, forecast_length=forecast_length)
    else:
        model = _load_legacy_model_via_direct_init(backend=backend, forecast_length=forecast_length)
...
def timesfm_1p0_200m_forecastor(dataframe, forecast_length, num_samples=100, freq=None, device=None):
