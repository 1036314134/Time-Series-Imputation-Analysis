...
def _load_timesfm_forecastor():
    module_path = Path(__file__).resolve().parent / "timesfm_1p0_200m_forecastor.py"
    module_name = "timesfm_1p0_200m_forecastor_module"
    ...
    if not hasattr(module, "timesfm_1p0_200m_forecastor"):
        raise AttributeError("timesfm_1p0_200m_forecastor.py does not define timesfm_1p0_200m_forecastor.")

    return module.timesfm_1p0_200m_forecastor


timesfm_1p0_200m_forecastor = _load_timesfm_forecastor()
...
def sliding_window_forecast_test(
    ...
    for window_index, offset in enumerate(range(0, test_length, forecast_window), start=1):
        ...
        forecast_df = timesfm_1p0_200m_forecastor(
...
def use_timesfm_1p0_200m_forecastor(
    csv_relative_path,
...
if __name__ == "__main__":
    result_df, window_df = use_timesfm_1p0_200m_forecastor(csv_relative_path="dataset/exchange_rate/exchange_rate.csv")
    print(f"Generated {len(result_df)} predictions across {len(window_df)} windows.")
    save_dataframe_to_csv(result_df, "results/exchange_rate_predictions_timesfm_1p0_200m.csv")
    save_dataframe_to_csv(window_df, "results/exchange_rate_windows_timesfm_1p0_200m.csv")
