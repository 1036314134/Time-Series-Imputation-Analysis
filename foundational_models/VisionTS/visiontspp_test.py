...
def _load_visiontspp_forecastor():
    module_path = Path(__file__).resolve().parent / "visiontspp_forecastor.py"
    module_name = "visiontspp_forecastor_module"
    ...
    if not hasattr(module, "visiontspp_forecastor"):
        raise AttributeError("visiontspp_forecastor.py does not define visiontspp_forecastor.")

    return module.visiontspp_forecastor


visiontspp_forecastor = _load_visiontspp_forecastor()
...
def sliding_window_forecast_test(
    ...
    for window_index, offset in enumerate(range(0, test_length, forecast_window), start=1):
        ...
        forecast_df = visiontspp_forecastor(
...
def test_visiontspp_forecastor(
    csv_relative_path,
...
if __name__ == "__main__":
    result_df, window_df = test_visiontspp_forecastor(csv_relative_path="dataset/exchange_rate/exchange_rate.csv")
    print(f"Generated {len(result_df)} predictions across {len(window_df)} windows.")
    save_dataframe_to_csv(result_df, "results/exchange_rate_predictions_visiontspp.csv")
    save_dataframe_to_csv(window_df, "results/exchange_rate_windows_visiontspp.csv")
