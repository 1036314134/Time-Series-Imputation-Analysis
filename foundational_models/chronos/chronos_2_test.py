...
def _load_chronos_forecastor():
    module_path = Path(__file__).resolve().parent / "chronos_2_forecastor.py"
    module_name = "chronos_2_forecastor_module"
    ...
    if not hasattr(module, "chronos_2_forecastor"):
        raise AttributeError("chronos_2_forecastor.py does not define chronos_2_forecastor.")

    return module.chronos_2_forecastor


chronos_2_forecastor = _load_chronos_forecastor()
...
if __name__ == "__main__":
    result_df, window_df = test_chronos_2_forecastor(csv_relative_path="dataset/exchange_rate/exchange_rate.csv")
    print(f"Generated {len(result_df)} predictions across {len(window_df)} windows.")
    save_dataframe_to_csv(result_df, "results/exchange_rate_predictions_chronos_2.csv")
    save_dataframe_to_csv(window_df, "results/exchange_rate_windows_chronos_2.csv")
