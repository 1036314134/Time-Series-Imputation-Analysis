...
def _load_moirai_forecastor():
    module_path = Path(__file__).resolve().parent / "moirai_2p0_r_small_forecastor.py"
    module_name = "moirai_2p0_r_small_forecastor_module"
    ...
    if not hasattr(module, "moirai_2p0_r_small_forecastor"):
        raise AttributeError("moirai_2p0_r_small_forecastor.py does not define moirai_2p0_r_small_forecastor.")

    return module.moirai_2p0_r_small_forecastor


moirai_2p0_r_small_forecastor = _load_moirai_forecastor()
...
def sliding_window_forecast_test(
    ...
    for window_index, offset in enumerate(range(0, test_length, forecast_window), start=1):
        ...
        forecast_df = moirai_2p0_r_small_forecastor(
...
def test_moirai_2p0_r_small_forecastor(
    csv_relative_path,
...
if __name__ == "__main__":
    result_df, window_df = test_moirai_2p0_r_small_forecastor(csv_relative_path="dataset/exchange_rate/exchange_rate.csv")
    print(f"Generated {len(result_df)} predictions across {len(window_df)} windows.")
    save_dataframe_to_csv(result_df, "results/exchange_rate_predictions_moirai_2p0_r_small.csv")
    save_dataframe_to_csv(window_df, "results/exchange_rate_windows_moirai_2p0_r_small.csv")
